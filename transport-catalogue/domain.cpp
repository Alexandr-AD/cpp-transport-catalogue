...
size_t StopsDistanceHasher::operator()(const std::pair<const Stop *, const Stop *> &p) const
{
 ...
 * визуализатор карты (MapRenderer) можно будет сделать независящим от транспортного справочника.
 *
